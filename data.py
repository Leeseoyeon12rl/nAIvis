...
def V2_get_continual_ember_class_data(data_dir, train=True):
    ...
    if train:
        #data_dir = data_dir + '/'
        #X Y_train = np.load(data_dir + 'XY_train.npz')
        #XY_train = np.load(os.path.join(data_dir, 'XY_train.npz'))
        #X_tr, Y_tr = XY_train['X_train'], XY_train['Y_train']

        with open('/home/swu/continual-learning-malware/dataset/X_train_CW_NoDef.pkl','rb')as handle:
            X_train=np.array(pickle.load(handle), dtype=object)

        with open('/home/swu/continual-learning-malware/dataset/y_train_CW_NoDef.pkl','rb') as handle:
            y_train=np.array(pickle.load(handle), dtype=object)
        ...
        with open('/home/swu/continual-learning-malware/dataset/X_test_CW_NoDef.pkl','rb') as handle:
            X_test=np.array(pickle.load(handle),dtype=object)

        with open('/home/swu/continual-learning-malware/dataset/y_test_CW_NoDef.pkl','rb') as handle:
            y_test=np.array(pickle.load(handle),dtype=object)
...
def get_malware_multitask_experiment(dataset_name, target_classes, init_classes,\
    ...
    if dataset_name == 'EMBER':
        ...
        data_dir = '/home/swu/continual-learning-malware/top_classes_100'
