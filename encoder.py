...
class DFNet(torch.nn.Module):
    ...
    def forward(self, x):
        x = x.view(x.shape[0], 1, -1)  # (batch_size, channels, feature_dim)
        x = x.to(self.device)
        ...
        print("DEBUG: Shape before FC layer:", x.shape)
        x = x.view(x.shape[0], -1)  # Flatten
        print("DEBUG: Shape after flatten:", x.shape)
    ...
    def train_a_batch(self, x, y, optimizer, criterion, device):
        self.train()
        optimizer.zero_grad()
