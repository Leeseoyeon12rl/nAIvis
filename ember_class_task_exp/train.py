...
import copy
import utils
from ember_class_task_exps.original_data import malwareSubDatasetExemplars as SubDataset
from ember_class_task_exps.original_data import ExemplarDataset
...
def custom_validate(model, dataset, batch_size=128, test_size=1024, verbose=True, allowed_classes=None,
...
def train_cl(model, model_save_path, train_datasets, test_datasets, args, replay_mode="none", scenario="class",classes_per_task=None,iters=2000,batch_size=32,
    ...
    for task, train_dataset in enumerate(train_datasets, 1):
        print(f"DEBUG: train_datasets length: {len(train_datasets)}")
        for i, dataset in enumerate(train_datasets):
            print(f"DEBUG: Task {i+1} dataset size: {len(dataset)}")
            ...
            if batch_index <= iters:
                ...
                if isinstance(model, DFNet):
                    loss_dict = model.train_a_batch(
                        x, y, optimizer=model.optimizer, criterion=torch.nn.CrossEntropyLoss(), device=model.device
                    )
                else:       
                    loss_dict = model.train_a_batch(
                        x, y, x_=x_, y_=y_, scores=scores, scores_=scores_,
                ...
                if hasattr(model, "label") and model.label == "VAE":
